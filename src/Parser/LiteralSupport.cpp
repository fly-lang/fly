...
static DiagnosticBuilder Diag(DiagnosticsEngine *Diags, FullSourceLoc TokLoc,
...
///         TODO: add rules...
///
NumericLiteralParser::NumericLiteralParser(SourceManager &SM, DiagnosticsEngine &Diags,
                                           StringRef TokSpelling,
                                           SourceLocation TokLoc)
  : SM(SM), Diags(Diags), ThisTokBegin(TokSpelling.begin()), ThisTokEnd(TokSpelling.end()) {
  ...
  if (s != ThisTokEnd || isImaginary) {
    ...
    if (isValidUDSuffix(UDSuffixBuf)) {
      if (!isImaginary) {
    ...
    if (s != ThisTokEnd) {
      // Report an error if there are any.
        Diags.Report(Lexer::AdvanceToTokenCharacter(TokLoc, SuffixBegin - ThisTokBegin, SM),
...
void NumericLiteralParser::ParseDecimalOrOctalCommon(SourceLocation TokLoc){
  ...
  if (isHexDigit(*s) && *s != 'e' && *s != 'E' &&
      !isValidUDSuffix(StringRef(s, ThisTokEnd - s))) {
  ...
  if (*s == 'e' || *s == 'E') { // exponent
    ...
    if (containsDigits(s, first_non_digit)) {
      ...
      if (!hadError) {
        Diags.Report(Lexer::AdvanceToTokenCharacter(TokLoc, Exponent-ThisTokBegin, SM),
...
bool NumericLiteralParser::isValidUDSuffix(StringRef Suffix) {
...
CharLiteralParser::CharLiteralParser(const char *begin, const char *end,
                                     SourceLocation Loc, SourceManager &SM, DiagnosticsEngine &Diags,
                                     TargetInfo &Target, tok::TokenKind kind)
                                     : SM(SM), Diags(Diags), Target(Target){
  ...
  assert(Target.getCharWidth() == 8 &&
         "Assumes char is 8 bits");
  assert(Target.getIntWidth() <= 64 &&
         (Target.getIntWidth() & 7) == 0 &&
         "Assumes sizeof(int) on target is <= 64 and a multiple of char");
  assert(Target.getWCharWidth() <= 64 &&
  ...
  while (begin != end) {
      ...
      if (res != llvm::conversionOK) {
        ...
        for (; tmp_out_start < buffer_begin; ++tmp_out_start) {
          if (*tmp_out_start > largest_character_for_kind) {
            HadError = true;
            Diags.Report(Loc, diag::err_character_too_large);
          }
        }
      }
    ...
    unsigned CharWidth = getCharWidth(Kind, Target);
    uint64_t result =
  ...
  if (NumCharsSoFar > 1) {
    if (isAscii() && NumCharsSoFar == 4)
      ...
      Diags.Report(Loc, diag::err_multichar_utf_character_literal);
    IsMultiChar = true;
  } else {
    IsMultiChar = false;
  }

  llvm::APInt LitVal(Target.getIntWidth(), 0);
  ...
  if (!HadError && multi_char_too_long) {
    Diags.Report(Loc, diag::warn_char_constant_too_large);
  }
...
StringLiteralParser(ArrayRef<Token> StringToks,
                    SourceLocation Loc, SourceManager &SM, DiagnosticsEngine &Diags,
                    TargetInfo &Target, bool Complain)
  : SM(SM), Target(Target), Diags(Complain ? &Diags :nullptr),
...
/// suffixes as ud-suffixes, because the diagnostic experience is better if we
/// treat it as an invalid suffix.
bool StringLiteralParser::isValidUDSuffix(StringRef Suffix) {
  return NumericLiteralParser::isValidUDSuffix(Suffix) ||
         Suffix == "sv";
}
