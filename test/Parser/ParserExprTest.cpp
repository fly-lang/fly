...
namespace {
    ...
    TEST_F(ParserTest, UnarySideExpr) {
        ...
        EXPECT_EQ(AddExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_ADD);
    ...
    TEST_F(ParserTest, BinaryAssignAddExpr) {
        ...
        EXPECT_EQ(As<ASTBinary>(ExprStmt->getExpr())->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_ADD);
    ...
    TEST_F(ParserTest, BinaryAssignSubExpr) {
        ...
        auto *ExprStmt = As<ASTExprStmt>(Body->getContent()[0]);
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        EXPECT_EQ(As<ASTBinary>(ExprStmt->getExpr())->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_SUB);
        auto *BinaryExpr = As<ASTBinary>(ExprStmt->getExpr());
    ...
    TEST_F(ParserTest, BinaryAssignMulExpr) {
        ...
        auto *ExprStmt = As<ASTExprStmt>(Body->getContent()[0]);
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        EXPECT_EQ(As<ASTBinary>(ExprStmt->getExpr())->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_MUL);
        auto *BinaryExpr = As<ASTBinary>(ExprStmt->getExpr());
    ...
    TEST_F(ParserTest, BinaryAssignDivExpr) {
        ...
        auto *ExprStmt = As<ASTExprStmt>(Body->getContent()[0]);
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        EXPECT_EQ(As<ASTBinary>(ExprStmt->getExpr())->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_DIV);
        auto *BinaryExpr = As<ASTBinary>(ExprStmt->getExpr());
    ...
    TEST_F(ParserTest, BinaryAssignModExpr) {
        ...
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        auto *BinaryExpr = As<ASTBinary>(ExprStmt->getExpr());
        EXPECT_EQ(BinaryExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_MOD);
    ...
    TEST_F(ParserTest, BinaryAssignAndExpr) {
        ...
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        auto *BinaryExpr = As<ASTBinary>(ExprStmt->getExpr());
        EXPECT_EQ(BinaryExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_AND);
    ...
    TEST_F(ParserTest, BinaryAssignOrExpr) {
        ...
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        auto *BinaryExpr = As<ASTBinary>(ExprStmt->getExpr());
        EXPECT_EQ(BinaryExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_OR);
    ...
    TEST_F(ParserTest, BinaryAssignXorExpr) {
        ...
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        auto *BinaryExpr = As<ASTBinary>(ExprStmt->getExpr());
        EXPECT_EQ(BinaryExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_XOR);
    ...
    TEST_F(ParserTest, BinaryAssignShiftLExpr) {
        ...
        auto *ExprStmt = As<ASTExprStmt>(Body->getContent()[0]);
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        EXPECT_EQ(As<ASTBinary>(ExprStmt->getExpr())->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_SHIFT_L);
        auto *BinaryExpr = As<ASTBinary>(ExprStmt->getExpr());
    ...
    TEST_F(ParserTest, BinaryAssignShiftRExpr) {
        ...
        auto *ExprStmt = As<ASTExprStmt>(Body->getContent()[0]);
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        EXPECT_EQ(As<ASTBinary>(ExprStmt->getExpr())->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN_SHIFT_R);
        auto *BinaryExpr = As<ASTBinary>(ExprStmt->getExpr());
    ...
    TEST_F(ParserTest, BinaryAddExpr) {
        ...
        auto *ExprStmt = As<ASTExprStmt>(Body->getContent()[0]);
        EXPECT_EQ(ExprStmt->getExpr()->getExprKind(), ASTExprKind::EXPR_BINARY);
        EXPECT_EQ(As<ASTBinary>(ExprStmt->getExpr())->getBinaryKind(), ASTBinaryKind::OP_BINARY_ASSIGN);
    ...
    TEST_F(ParserTest, BinarySubExpr) {
        ...
        EXPECT_EQ(SubExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_SUB);
    ...
    TEST_F(ParserTest, BinaryMulExpr) {
        ...
        EXPECT_EQ(MulExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_MUL);
    ...
    TEST_F(ParserTest, BinaryDivExpr) {
        ...
        EXPECT_EQ(DivExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_DIV);
    ...
    TEST_F(ParserTest, BinaryModExpr) {
        ...
        EXPECT_EQ(ModExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_MOD);
    ...
    TEST_F(ParserTest, BinaryAndExpr) {
        ...
        EXPECT_EQ(AndExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_AND);
    ...
    TEST_F(ParserTest, BinaryOrExpr) {
        ...
        EXPECT_EQ(OrExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_OR);
    ...
    TEST_F(ParserTest, BinaryXorExpr) {
        ...
        EXPECT_EQ(XorExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_XOR);
    ...
    TEST_F(ParserTest, BinaryShiftLExpr) {
        ...
        EXPECT_EQ(ShiftLExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_SHIFT_L);
    ...
    TEST_F(ParserTest, BinaryShiftRExpr) {
        ...
        EXPECT_EQ(ShiftRExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_SHIFT_R);
    ...
    TEST_F(ParserTest, BinaryLogicAndExpr) {
        ...
        EXPECT_EQ(LogicAndExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_LOGIC_AND);
    ...
    TEST_F(ParserTest, BinaryLogicOrExpr) {
        ...
        EXPECT_EQ(LogicOrExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_LOGIC_OR);
    ...
    TEST_F(ParserTest, BinaryComparisonEqualExpr) {
        ...
        EXPECT_EQ(CmpEqExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_COMPARE_EQ);
    ...
    TEST_F(ParserTest, BinaryComparisonNotEqualExpr) {
        ...
        EXPECT_EQ(CmpNeExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_COMPARE_NE);
    ...
    TEST_F(ParserTest, BinaryComparisonGreaterThanExpr) {
        ...
        EXPECT_EQ(CmpGtExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_COMPARE_GT);
    ...
    TEST_F(ParserTest, BinaryComparisonGreaterThanEqualExpr) {
        ...
        EXPECT_EQ(CmpGteExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_COMPARE_GTE);
    ...
    TEST_F(ParserTest, BinaryComparisonLessThanExpr) {
        ...
        EXPECT_EQ(CmpLtExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_COMPARE_LT);
    ...
    TEST_F(ParserTest, BinaryComparisonLessThanEqualExpr) {
        ...
        EXPECT_EQ(CmpLteExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_COMPARE_LTE);
    ...
    TEST_F(ParserTest, BinaryAddMulExpr) {
        ...
        EXPECT_EQ(BinaryAddExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_ADD);
        ...
        EXPECT_EQ(BinaryMulExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_MUL);
    ...
    TEST_F(ParserTest, BinarySubDivExpr) {
        ...
        EXPECT_EQ(BinarySubExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_SUB);
        ...
        EXPECT_EQ(BinaryDivExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_DIV);
    ...
    TEST_F(ParserTest, BinaryParenExpr) {
        ...
        EXPECT_EQ(BinaryModExpr->getBinaryKind(), ASTBinaryKind::OP_BINARY_ARITH_MOD);
