...
/* mem_alloc_zeroed returns ZEROED memory (Rust's GlobalAlloc::alloc_zeroed).
 * On the CRT this is calloc; on Windows Rust uses HeapAlloc(HEAP_ZERO_MEMORY),
 * but we stay on the CRT (malloc/free-compatible) so calloc is the equivalent. */
