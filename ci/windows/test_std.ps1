...
# suite reports twice in the merged log. Per-suite loop kept for
# FLY_TEST_PER_SUITE=1 (debugging: one exe, log and exit code per suite). It no
# longer covers STAGE=0 — the seed has no test runner since 0.13.15, so no stage
# runs the suites with it.
$oneShot = ($env:FLY_TEST_PER_SUITE -ne '1')
